...
自托管 AI 盯盘助手，覆盖 A 股 / 港股 / 美股，集成多 Agent 投资决策、事件日历、板块轮动、X 舆论监控与全渠道推送。
...
[![Docker Build](https://img.shields.io/badge/docker-ghcr.io%2Fwwkisu%2Ffinanceassistant-blue?logo=docker)](https://github.com/jinting1996/FinanceAssistant/pkgs/container/financeassistant)
...
盯盘侠不是一个简单的指标看板。它把数据采集、AI 分析、交易策略和消息推送串联成一个完整的决策闭环，跑在你自己的服务器上，数据不经过任何第三方。

- **数据私有** — 自托管部署，持仓数据完全由你掌控
- **AI 原生** — 不是指标堆砌，而是让 AI 理解你的持仓、风格和目标，输出可执行的建议
- **事件驱动** — 重大消息、板块日 K、技术指标和资金轮动放在同一个页面判断，减少信息跳转
- **开箱即用** — 本地 `make dev-api && make dev-web`，5 分钟完成配置；Docker 一条命令拉起
- **个人维护** — 本仓库由 [PotatoChipking](https://github.com/PotatoChipking) 维护，持续迭代
...
接入 [TradingAgents](https://github.com/TauricResearch/TradingAgents)（76k+ star）多 Agent 投资决策框架。点击持仓页或事件日历中任意股票的 🧠 图标即可触发，3-5 分钟输出完整推理链：

分析师团队（技术面 / 情绪面 / 新闻面 / 基本面）→ 多空辩论 → 风控审查 → PM 整合决策

结论同步推送到 Telegram / 企业微信 / 钉钉 / 飞书。支持未加入自选池的板块龙头股临时分析，适合从事件和板块轮动里快速下钻。默认使用 deepseek-chat，单次约 $0.05。
...
- **量价分析**：量比异动、缩量回调、放量突破
- **Price Action**：识别 20 日高点放量突破、回踩确认、上升趋势结构，生成 ATR 止损与风险收益目标位
- **形态识别**：锤子线、吞没形态、十字星等 K 线形态
- **支撑压力**：自动计算多级支撑位和压力位

### 底仓 VWAP 回归做 T

仅扫描已有 A 股持仓，需在持仓编辑中维护可卖底仓和账户可用资金。日线趋势未破、接近支撑、低于当日 VWAP、分钟 K 止跌且 T Score 达到 70 时才触发提醒。收到低吸提醒后需在持仓页确认买入，系统才会继续监控卖点。单次默认使用可卖底仓 20%，按 100 股取整，每股每天最多完成一次，跌破止损或信号超时自动失效。只生成提醒，不连接券商、不自动下单。

### 价格提醒

支持价格、涨跌幅、成交额、量比、PA 评分/突破/回踩/结构失效等条件组合（AND/OR）。支持交易时段/全天生效、冷却时间、日触发上限、重复触发模式，可按规则选择通知渠道。

### X (Twitter) 舆论监控

基于 `twikit` 免费抓取 X 上的 Cashtag（`$SYMBOL`）讨论，通过 AI 进行正面/负面/中性情感分类。分析结果汇总为结构化舆情数据，可作为 Agent 的补充数据源。需要配置 X 账号（建议使用小号）。
...
| 类型 | 数据源 | 状态 |
|------|--------|------|
| 新闻 | 雪球资讯、东方财富资讯、东方财富公告 | 可选启用 |
| K 线 | 腾讯 K 线、东方财富 K 线、Stooq、Tushare、YFinance | 多源备选 |
| 资金流向 | 东方财富资金流 | 默认启用 |
| 行情 | 腾讯行情 | 默认启用 |
| 事件 | 东方财富事件 | 默认启用 |
...
覆盖 A 股、港股、美股实时行情，支持多券商账户独立管理、汇总展示总资产，按短线/波段/长线分别设置交易风格。
...
访问 `http://localhost:5183`，首次使用设置账号密码后即可进入。
...
浏览器打开 `http://<服务器IP>:8000`。数据（SQLite、Playwright 浏览器、运行时文件）持久化在 `panwatch_data` 卷中，升级不丢失。
...
### 从 Docker Hub 拉取（国内友好）

预构建镜像已推送至 Docker Hub，适合国内网络环境：

```bash
docker pull wwkisu/financeassistant:latest
```

镜像同时发布到 GitHub Container Registry：
...
项目根目录提供了 `docker-compose.yml` 配置参考，可直接用于绿联 NAS 等 Docker 设备。将镜像地址改为 `wwkisu/financeassistant:latest`，按需修改端口和数据卷路径即可。
...
WSL 中部署时，`127.0.0.1` 指向的是 WSL 自身而非宿主机。需要在 WSL 中查看宿主机地址：
...
详见 [`.env.example`](.env.example) 和 [TradingAgents 配置指南](.docs/tradingagents/USER_GUIDE.md)。
